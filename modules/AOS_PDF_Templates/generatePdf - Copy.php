	...
	if($firstValue !== '' && $lastvalue !== ''){
		...
		if(count($lineItems) != 0){
			...
			foreach($lineItems as $id => $productId){
				...
				$text .= $sep . templateParser::parse_template($linePart, $obb, $SummaryDetails); 
...
// echo $text; die;  CalculatedTaxAmount
 ...
 //}
	...
	$converted = templateParser::parse_template($text, $object_arr, $SummaryDetails);
    $converted =  html_entity_decode(str_replace('&nbsp;',' ',$converted));
	$header = templateParser::parse_template($header, $object_arr, $SummaryDetails);
	$footer = templateParser::parse_template($footer, $object_arr, $SummaryDetails);
	...
	if($task == 'pdf' || $task == 'emailpdf')
	{
		...
		try{ 
			...
			$pdf->SetHTMLFooter($footer.'&nbsp; Page No: {PAGENO}'); 
