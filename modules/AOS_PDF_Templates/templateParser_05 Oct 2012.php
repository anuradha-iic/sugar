...
class templateParser{
		function parse_template_bean($string, $bean_name, &$focus, &$SummaryDetails) {
		global $app_strings;
			$repl_arr = array();
			...
			foreach ($repl_arr as $name=>$value) {  
				...
				if ( ($name === 'aos_quotes_product_subtotal_mrc') or ($name === 'aos_quotes_order_discount') or ($name === 'aos_quotes_order_nrc_discont') or ($name === 'aos_quotes_product_subtotal_lmd') ){  
					$value = currency_format_number($value,$params = array('currency_symbol' => true, 'decimals'=> 2));					
				} 		
		...
		function parse_template($string, &$bean_arr, &$SummaryDetails) {
			...
			foreach($bean_arr as $bean_name => $bean_id) {
				...
				$string = templateParser::parse_template_bean($string, $bean_name, $focus, $SummaryDetails);
			}
			return $string;
		}
	}
