...
{/if}  
<input type='text' name='{$fields.maincode.name}' 
id='{$fields.maincode.name}' size='30' 
maxlength='100' 
...
{if !isset($config.enable_autocomplete) || $config.enable_autocomplete==false}
<select name="{$fields.unit_measure.name}" 
id="{$fields.unit_measure.name}" 
title='' tabindex="105"  
>
{if isset($fields.unit_measure.value) && $fields.unit_measure.value != ''}
{html_options options=$fields.unit_measure.options selected=$fields.unit_measure.value}
{else}
{html_options options=$fields.unit_measure.options selected=$fields.unit_measure.default}
{/if}
</select>
{else}
{assign var="field_options" value=$fields.unit_measure.options }
{capture name="field_val"}{$fields.unit_measure.value}{/capture}
{assign var="field_val" value=$smarty.capture.field_val}
{capture name="ac_key"}{$fields.unit_measure.name}{/capture}
{assign var="ac_key" value=$smarty.capture.ac_key}
<select style='display:none' name="{$fields.unit_measure.name}" 
id="{$fields.unit_measure.name}" 
title='' tabindex="105"  
>
{if isset($fields.unit_measure.value) && $fields.unit_measure.value != ''}
...
<input
id="{$fields.unit_measure.name}-input"
name="{$fields.unit_measure.name}-input"
size="30"
value="{$field_val|lookup:$field_options}"
type="text" style="vertical-align: top;">
<span class="id-ff multiple">
<button type="button"><img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.unit_measure.name}-image"></button><button type="button"
id="btn-clear-{$fields.unit_measure.name}-input"
title="Clear"
onclick="SUGAR.clearRelateField(this.form, '{$fields.unit_measure.name}-input', '{$fields.unit_measure.name}');sync_{$fields.unit_measure.name}()"><img src="{sugar_getimagepath file="id-ff-clear.png"}"></button>
</span>
{literal}
<script>
SUGAR.AutoComplete.{/literal}{$ac_key}{literal} = [];
...
{literal}
(function (){
var selectElem = document.getElementById("{/literal}{$fields.unit_measure.name}{literal}");
if (typeof select_defaults =="undefined")
select_defaults = [];
select_defaults[selectElem.id] = {key:selectElem.value,text:''};
//get default
...
YUI().use("autocomplete", "autocomplete-filters", "autocomplete-highlighters", "node","node-event-simulate", function (Y) {
{/literal}
SUGAR.AutoComplete.{$ac_key}.inputNode = Y.one('#{$fields.unit_measure.name}-input');
SUGAR.AutoComplete.{$ac_key}.inputImage = Y.one('#{$fields.unit_measure.name}-image');
SUGAR.AutoComplete.{$ac_key}.inputHidden = Y.one('#{$fields.unit_measure.name}');
{literal}
function SyncToHidden(selectme){
var selectElem = document.getElementById("{/literal}{$fields.unit_measure.name}{literal}");
var doSimulateChange = false;
...
//global variable 
sync_{/literal}{$fields.unit_measure.name}{literal} = function(){
SyncToHidden();
}
function syncFromHiddenToWidget(){
var selectElem = document.getElementById("{/literal}{$fields.unit_measure.name}{literal}");
...
SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.simulate('keyup');
for (i=0;i<selectElem.options.length;i++){
if (selectElem.options[i].value==selectElem.value && document.activeElement != document.getElementById('{/literal}{$fields.unit_measure.name}-input{literal}'))
SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.set('value',selectElem.options[i].innerHTML);
}
}
YAHOO.util.Event.onAvailable("{/literal}{$fields.unit_measure.name}{literal}", syncFromHiddenToWidget);
{/literal}
SUGAR.AutoComplete.{$ac_key}.minQLen = 0;
SUGAR.AutoComplete.{$ac_key}.queryDelay = 0;
SUGAR.AutoComplete.{$ac_key}.numOptions = {$field_options|@count};
...
id='{$fields.cost.name}' size='30' maxlength='26' value='{sugar_number_format var=$value}' title='' tabindex='107' >
</tr>
{/capture}
{if $fieldsUsed > 0 }
{$tableRow}
...
id='{$fields.price.name}' size='30' maxlength='26' value='{sugar_number_format var=$value}' title='' tabindex='108' >
</tr>
{/capture}
{if $fieldsUsed > 0 }
{$tableRow}
...
<input type="text" name="{$fields.contact.name}" class="sqsEnabled" tabindex="110" id="{$fields.contact.name}" size="" value="{$fields.contact.value}" title='' autocomplete="off"  >
<input type="hidden" name="{$fields.contact.id_name}" 
id="{$fields.contact.id_name}" 
value="{$fields.contact_id.value}">
<span class="id-ff multiple">
<button type="button" name="btn_{$fields.contact.name}" id="btn_{$fields.contact.name}" tabindex="110" title="{sugar_translate label="LBL_ACCESSKEY_SELECT_CONTACTS_TITLE"}" accessKey="{sugar_translate label="LBL_ACCESSKEY_SELECT_CONTACTS_KEY"}" class="button firstChild" value="{sugar_translate label="LBL_ACCESSKEY_SELECT_CONTACTS_LABEL"}" 
onclick='open_popup(
...
"single", 
true
);' ><img src="{sugar_getimagepath file="id-ff-select.png"}"></button><button type="button" name="btn_clr_{$fields.contact.name}" id="btn_clr_{$fields.contact.name}" tabindex="110" title="{sugar_translate label="LBL_ACCESSKEY_CLEAR_CONTACTS_TITLE"}" accessKey="{sugar_translate label="LBL_ACCESSKEY_CLEAR_CONTACTS_KEY"}" class="button lastChild" 
...
addToValidate('EditView', 'maincode', 'varchar', true,'{/literal}{sugar_translate label='LBL_MAINCODE' module='AOS_Products' for_js=true}{literal}' );
...
addToValidate('EditView', 'cost', 'currency', true,'{/literal}{sugar_translate label='LBL_COST' module='AOS_Products' for_js=true}{literal}' );
...
addToValidate('EditView', 'unit_measure', 'enum', false,'{/literal}{sugar_translate label='LBL_UNIT_MEASURE' module='AOS_Products' for_js=true}{literal}' );
addToValidate('EditView', 'jbilling_relation_id_c', 'varchar', false,'{/literal}{sugar_translate label='LBL_JBILLING_RELATION_ID' module='AOS_Products' for_js=true}{literal}' );
addToValidate('EditView', 'homir_id_c', 'varchar', false,'{/literal}{sugar_translate label='LBL_HOMIR_ID' module='AOS_Products' for_js=true}{literal}' );
