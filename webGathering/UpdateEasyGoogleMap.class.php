...
class EasyGoogleMap {
	 ...
	 */
	var $mMapType = TRUE;
        var $mMapTypeId = 'G_SATELLITE_MAP';
	 ...
	 * @return: void
	 */
	function SetMapWidth($width=407) {
	...
	function InitJs($counter) {
		...
		if ($is_error) { 
			...
			$ret .= "var gmarkers$counter = [];\n";
			$ret .= "var address$counter = [];\n";
			$ret .= "var points$counter = [];\n";
				
			$ret .= "if(GBrowserIsCompatible()) { \n"; 
                        $ret .= "	var map$counter = new GMap2(document.getElementById('updatemap".$counter."')); \n";
	                $ret .= "	map$counter.setMapType(".$this->mMapTypeId."); \n";
			# handle map continuous zooming
			$ret .= ($this->mContinuousZoom==TRUE)?"	map$counter.enableContinuousZoom(); \n":"";
	
			# handle map double click zooming
			$ret .= ($this->mDoubleClickZoom==TRUE)?"	map$counter.enableDoubleClickZoom(); \n":"";
			...
			switch ($this->mDefControl) {
				...
				case 'SMALL_PAN_ZOOM':
					$mapCtrl = "map$counter.addControl(new GSmallMapControl()); \n";
					break;
					
				case 'LARGE_PAN_ZOOM':
					$mapCtrl = "map$counter.addControl(new GLargeMapControl()); \n";
					break;
	
				case 'SMALL_ZOOM':
					$mapCtrl = "map$counter.addControl(new GSmallZoomControl()); \n";
					break;
			...
			$ret .= ($this->mScale==TRUE)?"	map$counter.addControl(new GScaleControl()); \n":"";
	
			# handle map type (map/satellite/hybrid)
			$ret .= ($this->mMapType==TRUE)?"	map$counter.addControl(new GMapTypeControl()); \n":"";
	
			# handle map inset
			$ret .= ($this->mInset==TRUE)?"	map$counter.addControl(new GOverviewMapControl()); \n":"";
	
			$ret .= "	var geocoder$counter = new GClientGeocoder(); \n";
			$ret .= "	var icon$counter = new GIcon(); \n";
			$ret .= "	icon$counter.image = 'http://google.webassist.com/google/markers/$dir/$color.png'; \n";
			$ret .= "	icon$counter.shadow = 'http://google.webassist.com/google/markers/$dir/shadow.png'; \n";
			$ret .= "	icon$counter.iconSize = new GSize($icon_w,$icon_h); \n";
			$ret .= "	icon$counter.shadowSize = new GSize($icon_w,$icon_h); \n";
			$ret .= "	icon$counter.iconAnchor = new GPoint($icon_anchr_w,$icon_anchr_h); \n";
			$ret .= "	icon$counter.infoWindowAnchor = new GPoint($info_win_anchr_w,$info_win_anchr_h); \n";
			$ret .= "	icon$counter.printImage = 'http://google.webassist.com/google/markers/$dir/$color.gif'; \n";
			$ret .= "	icon$counter.mozPrintImage = 'http://google.webassist.com/google/markers/$dir/{$color}_mozprint.png'; \n";
			$ret .= "	icon$counter.printShadow = 'http://google.webassist.com/google/markers/$dir/shadow.gif'; \n";
			$ret .= "	icon$counter.transparent = 'http://google.webassist.com/google/markers/$dir/{$color}_transparent.png'; \n\n";

			# loop set address(es)
			for ($i=$cnt_add-1; $i>=0; $i--) {
			
				$ret .= "	var address".$counter."_$i = {\n";
				$ret .= "	  infowindowtext: '".addslashes($this->mInfoWindowTextArr[$i])."',\n";
				$ret .= "	  full: '".addslashes($this->mAddressArr[$i])."'\n";
				$ret .= "	};\n\n";

				$ret .= "	address".$counter."[$i] = address".$counter."_$i.infowindowtext;\n\n";
				
				$ret .= "	geocoder$counter.getLatLng (\n";
				$ret .= "	  address".$counter."_$i.full,\n";
				$ret .= "	  function(point) {\n ";
				$ret .= "		if(point) {\n";
				$ret .= "		  points".$counter."[$i] = point; \n";	
				$ret .= "		  map$counter.setCenter(point, {$this->mMapZoom});\n";
				$ret .= "		  var marker$counter = new GMarker(point, icon$counter);\n";
				$ret .= "		  GEvent.addListener(marker$counter, 'click', function() {\n";
				$ret .= "			marker$counter.openInfoWindowHtml(address".$counter."_$i.infowindowtext);\n";
				$ret .= "		  });\n";

				$ret .= "		  map$counter.addOverlay(marker$counter);\n";

				# show only info window to the first set address
				if ($i===0) 
					$ret .= "		  marker$counter.openInfoWindowHtml(address".$counter."_$i.infowindowtext);\n";
				
				$ret .= "		  gmarkers$counter[$i] = marker$counter;\n";

				$ret .= "		}\n";
				$ret .= "		else {\n";
                                $ret .= "		     alert('Your specified location is invalid....'); \n";
				$ret .= "		  //map$counter.setCenter(new GLatLng(37.4419, -122.1419), {$this->mMapZoom});\n";
				$ret .= "		}\n";
				$ret .= "	  }\n";
				$ret .= "	); // end geocoder$counter.getLatLng\n\n";
			
			} # end for
			$ret .= "} // end if\n\n";
			
			$ret .= "function sideClick(i) {\n";
			$ret .= "   if (gmarkers$counter[i]) {\n";
			$ret .= "	  gmarkers$counter[i].openInfoWindowHtml(address".$counter."_[i]);\n";
			$ret .= "	  map$counter.setCenter(points".$counter."[i],{$this->mMapZoom});\n";
			$ret .= "   } else {\n";
			$ret .= "	  var htstring = address".$counter."[i];\n";
	 ...
	 * @return: string
	 */
	function MapHolder($counter) {
		return "<div id='map".$counter."' style=\"width: ".$this->mMapWidth."px; height: ".$this->mMapHeight."px;\"></div>";
	} # end function
