...
function calculateDiscount(ln)
{
	...
	document.getElementById('product_discount_amount' + ln).value = formatNumber(discount);
	 // 
	//document.getElementById('order_discount').value = formatNumber(discount);
	...
	calculateSubTotals();
	//document.getElementById('order_discount').value = formatNumber(document.getElementById('product_subtotal_discount').value); 
...
function insertProductLine(ln)
{
   	...
   	sqs_objects["product_name["+ln+"]"]=
	{
   		...
   		"populate_list":["product_description["+ln+"]", "product_name["+ln+"]","product_id["+ln+"]","product_cost_price["+ln+"]","product_list_price["+ln+"]","unit["+ln+"]","product_code["+ln+"]"],
...
//	e.innerHTML="<input type='text' name='product_unit_price[]' id='product_unit_price" + ln + "' size='10' maxlength='50' value='' title='' tabindex='116' onfocus='calculateDiscount(" + ln + ");' onblur='calculateDiscount(" + ln + "); calculateProductLine(" + ln + ");'>";
	...
	//i.innerHTML = SUGAR.language.get(module, 'LBL_PRODUCT_QUOTE_DESC') +" <br/><br/>&nbsp;&nbsp;&nbsp;" +SUGAR.language.get(module, 'LBL_PRODUCT_QUOTE_DESCRIPTION') +" &nbsp;<br/><br/>&nbsp;&nbsp;&nbsp;&nbsp;" +SUGAR.language.get(module, 'LBL_PRODUCT_NOTE')+" :&nbsp";
	
	i.innerHTML = '<table cellspacing="3" cellpadding="0" border="0"><tbody><tr><td align="right" style="border-bottom: 1px solid #CBDAE6;" class="dataField">'+SUGAR.language.get(module, 'LBL_PRODUCT_QUOTE_DESC')+':&nbsp;</td></tr><tr><td align="right" class="dataField" style="border-bottom: 1px solid #CBDAE6;">'+SUGAR.language.get(module, 'LBL_PRODUCT_QUOTE_DESCRIPTION')+'&nbsp;</td></tr><tr style="border: none !important;"><td align="right" class="" style="border: none !important;">&nbsp;</td></tr><tr style="border: none !important;"><td align="right" class="" style="border: none !important;">&nbsp;</td></tr><tr><td align="right" class="dataField" style="border-bottom: 1px solid #CBDAE6;">'+SUGAR.language.get(module, 'LBL_PRODUCT_NOTE')+':&nbsp;</td></tr></tbody></table>';
	
	j.innerHTML="<input type='hidden' name='product_descriptionFetch[]' id='product_descriptionFetch"+ln+"' value=''><input type='text' name='quotes_product_name[]'  id='quotes_product_name"+ln+"' />   <br> <textarea tabindex='116' name='quotes_product_description[]' id='quotes_product_description" + ln + "' rows='2' cols='53'></textarea><br><textarea tabindex='116' name='product_note[]' id='product_note" + ln + "' rows='2' cols='53'></textarea>&nbsp;&nbsp;";
...
{  
	fromPopupReturn = true;
	var valarr = popupReplyData.form_name;
    var res1 = valarr.split(':');
	var productID;   
	var formName = res1[0];
	var counter  = res1[1];
	var nameToValueArray = popupReplyData.name_to_value_array; 
	...
	{   
		...
		{  
			...
			if(theKey.indexOf('product_id') != -1){ 
...
//	 subcode1(document.getElementById('product_id' + lineno).value);
     // counter  document.getElementById('quotes_product_description' + counter).value = 'dcss';
     var dbconfigVars  = document.getElementById('dbconfigVars').value; 	 
	 var params = "dbconfigVars="+dbconfigVars+"&counter="+counter+"&productID="+productID;   
	$.ajax({
		      type: 'POST',
			  data: params,
			  url: '/custom/modules/AOS_Quotes/views/view.fill.desc.prod.php',
			  success: function(data) { 
			          document.getElementById('quotes_product_description' + counter).value = data;
			  },
			  error: function(data) { 
...
{calculateSubTotals()
    ...
    var grandSectionsubtotal = subtotal - document.getElementById('order_discount').value;	
		
	document.getElementById('subtotal_amount').value = formatNumber(grandSectionsubtotal);
...
function getservice_address_call() {
	...
	var nli_serviceaddresses_id_c = document.getElementById("nli_serviceaddresses_id_c").value;
	if(serviceaddress_c != '') {
	   $("input[rel=service_address_rel]").each(function() {
		   if($(this).val() == "") {
		      var id = $(this).attr("id");
			  var temp = id.replace("service_address_name","service_address_id");
		      $(this).val(serviceaddress_c);
			  $("#"+temp).val(nli_serviceaddresses_id_c);
		   }
	   });
	}
...
var totalcheckuncheck = "";

function checkfor(obj)
{ 
     totalcheckuncheck += obj.value+' ';
}

function checkfor1(obj)
{  
	 var array2 = totalcheckuncheck.split(' ');
	 var counttotal = array2.length;
	 var tempstr = "";
	 
	 for(i=0; i<counttotal; i++)
	 {
	     if(array2[i] != obj.value)
		 {
		     tempstr += array2[i]+' ';
		 }
	 }
	 
	 totalcheckuncheck = tempstr;
}

function addContactsByAccount()
{   
      document.getElementById('addcontacts').innerHTML = "<img src='http://sugarcrm-dev.nextlevelinternet.com/load-img/ajax-loader.gif' border='0'>";
       
	 /* $('#btn_billing_account').click(function() {
	       document.getElementById('addcontacts').innerHTML = "<img src='http://sugarcrm-dev.nextlevelinternet.com/load-img/ajax-loader.gif' border='0'>"; 
	  });  */
	  
      var account_name  = document.getElementById('billing_account').value;
      var account_id    = document.getElementById('billing_account_id').value; 
	  var dbconfigVars  = document.getElementById('dbconfigVars').value; 
	  var quotesID4Ajax = document.getElementById('quotesID4Ajax').value; 
      //document.getElementById('addcontacts').innerHTML = document.getElementById('billing_account').value;  
	    var params = "dbconfigVars="+dbconfigVars+"&account_id="+account_id+"&totalcheckuncheck="+totalcheckuncheck+"&quotesID4Ajax="+quotesID4Ajax;  
	  ...
	  { 
		  $.ajax({
		      type: 'POST',
			  data: params,
			  url: '/custom/modules/AOS_Quotes/views/view.ajax.php',
			  success: function(data) { 
			       var res = data.split("||"); 
				   document.getElementById('addcontacts').innerHTML = "";
				   document.getElementById('addcontacts').innerHTML = res[0]; 
				   document.getElementById('totalAccountContacts').value = res[1];
				   document.getElementById('checkforContacts').value = res[2];
			  },
			  error: function(data) { 
				  //alert('There is an error.');
			  }
	 ...
	 // var test = setTimeout(function(){ addContactsByAccount(); }, 15000); 
}

$('#EditView').submit(function() {
 var totalcontactsID = "";
 
     var totalcontacts = document.getElementById('totalAccountContacts').value;   
	    if(totalcontacts > 0)
		{
		    for(i=0; i<totalcontacts; i++)
			 {
			      if( document.getElementById('contactsdisplay_'+i).checked == true )
				  { 
                           totalcontactsID += document.getElementById('contactsdisplay_'+i).value+",";					   
				  } 
			 }
		}
	 var account_id   = document.getElementById('billing_account_id').value; 
     var totalcount   = document.getElementById('totalAccountContacts').value;
	 var checkforContacts = document.getElementById('checkforContacts').value;
	  
	 
	//if(account_id != "" && totalcontactsID == "")
    // {
	     // if(checkforContacts == "no")
		 // { 
		      document.getElementById('totalcheckuncheck').value = totalcheckuncheck;  
		    // return true;
		//  }
		//  else
		//  {
	       // alert('Please wait until contacts of Acocunts will be listed');
           // return false;
		//  }	
   //  } 	
	//else
	// {
	       document.getElementById('selectedAccountContacts').value = totalcontactsID; 
	       return true;
	// }
	//return false; 
});
  ...
  if( $('#editcase').val() == '')  
   {
	   ...
	   $("#billing_contact").val(opportContactDetails["name"]);
	   $("#billing_contact_id").val(opportContactDetails["ContactID"]);
   }
   if( $('#editcase').val() == '')
   {
   ...
   //var test = setTimeout(function(){ addContactsByAccount(); }, 15000); 
    ...
    document.getElementById('LBL_EMAIL_ADDRESSES').innerHTML = document.getElementById('LBL_EMAIL_ADDRESSES').innerHTML + newdivtoadd;
