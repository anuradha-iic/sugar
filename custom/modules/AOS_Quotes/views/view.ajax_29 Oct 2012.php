...
class AOS_QuotesViewAjax { 
	...
	{   
		  ...
		  {
		        $query = "SELECT ctc.id, ctc.first_name, ctc.last_name, ctc.phone_work, ctc.phone_mobile, ctcst.opportunity_role_c FROM contacts as ctc INNER JOIN contacts_cstm as ctcst ON ctc.id = ctcst.id_c WHERE id='".$val."' AND deleted=0"; 
				 $result = mysql_query($query, $link);
				 $arrayD = array();
				 while ($row = mysql_fetch_assoc($result)) { 
