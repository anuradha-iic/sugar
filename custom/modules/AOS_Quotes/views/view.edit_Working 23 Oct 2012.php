...
class AOS_QuotesViewEdit extends ViewEdit {
	...
	{ 
        ...
        $sql = "SELECT qsc.contacts_id, ctc.first_name, ctc.last_name FROM quotes_saved_contacts as qsc INNER JOIN contacts as ctc ON qsc.contacts_id = ctc.id WHERE quotes_id = '".$quoteID."' and ctc.deleted = 0 ORDER BY ctc.first_name, ctc.last_name desc"; 
		$res = $this->bean->db->query($sql);
		while($row = $this->bean->db->fetchByAssoc($res)){ 
...
$dbconfigVars = $dbconfig['db_host_name'].";".$dbconfig['db_user_name'].";".$dbconfig['db_password'].";".$dbconfig['db_name'];		  
		...
		{    // Edit Quotes
			...
			if($totalContacts > 0)
			{
			   $html .= "<div id='addcontacts'>";
			   $html .= "<table cellpadding='0' cellspacing='0' border='0' width='126%'>
							    <tr>
								   <td colspan='2' style='font-size: 12pt; font-weight: bold;' valign='top'>
										The below are the already selected contacts 
								   </td>
								</tr>
			            ";  
                       ...
                       $html .= "<tr>
			             ...
			             <tr>
			                        <td valign='top' style='width: 4%'>";  
									  $html .= "<input type='checkbox' checked='checked' onclick='checkfor(this);' name='contactsdisplay_".$key."' id='contactsdisplay_".$key."' value='".$val['contacts_id']."'> "; 	 
			            $html .= "  </td>
		...
		$newdivtoadd = $html; ?>
		    <script language="Javascript">
			     var newdivtoadd = '<?php echo $newdivtoadd; ?>';
			</script>
		<?php		
