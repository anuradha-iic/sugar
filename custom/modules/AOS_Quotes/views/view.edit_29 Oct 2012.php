...
class AOS_QuotesViewEdit extends ViewEdit {
	...
	{ 
		...
		$res = $this->bean->db->query($sql);
		while($row = $this->bean->db->fetchByAssoc($res)){ 
		    $tmpArray['contacts_id']   = $row['contacts_id'];  
			$tmpArray['id']   = $row['id'];  
			$tmpArray['contact_name']  = $row['first_name'].' '.$row['last_name'];
			$tmpArray['phone_work']    = $row['phone_work'];
			$tmpArray['phone_mobile']  = $row['phone_mobile']; 
			...
			$tmpArray['opportunity_role_c'] = $row['opportunity_role_c'];
			   ...
			   $tempdivHTMLtoadd .= '<table cellpadding="0" cellspacing="0" border="0" width="126%"><tr><td colspan="6" style="font-size: 12pt; font-weight: bold;" valign="top">Select the Contacts to show on the SOF template</td></tr> <tr> <td colspan="6" style="font-size: 12pt; font-weight: bold;" valign="top">&nbsp;</td></tr> <tr> <th style="text-align: left;">&nbsp;</th> <th style="text-align: left;">Role</th> <th style="text-align: left;">Name</th> <th style="text-align: left;">Work</th> <th style="text-align: left;">Mobile</th> <th style="text-align: left;">Email</th> </tr>';   
				 foreach($contacts as $key=>$val)
		           { 	
			           ...
			           $tempdivHTMLtoadd .= '</td><td valign="top" style="width: 14%">'.chunk_split($opportunity_role_c, 18, "<br />").'</td><td valign="top" style="width: 14%"> <a href="http://sugarcrm-dev.nextlevelinternet.com/index.php?module=Contacts&return_module=Contacts&action=EditView&record='.$val['id'].'"> '.$val['contact_name'].' </a></td>';
