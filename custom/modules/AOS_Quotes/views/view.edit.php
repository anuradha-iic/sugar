...
class AOS_QuotesViewEdit extends ViewEdit {
	...
	function fetchContactIdsByAccountIDNew($accountID)
	{   
	    $query = "SELECT * FROM accounts_contacts WHERE account_id='".$accountID."' AND deleted=0";
	...
	function fetchContactDetailsByContactID($contactIDs)
	{   
	      foreach($contactIDs as $key=>$val)
		  {
		        $query = "SELECT ctc.id, ctc.first_name, ctc.last_name, ctc.phone_work, ctc.phone_mobile, ctcst.opportunity_role_c, ctcst.contact_type_primary_billing_c, ctcst.contact_type_other_billing_c, ctcst.contact_type_primary_tech_c, ctcst.contact_type_additional_tech_c, ctcst.contact_type_other_c FROM contacts as ctc INNER JOIN contacts_cstm as ctcst ON ctc.id = ctcst.id_c WHERE ctc.id='".$val."' AND ctc.deleted=0";  
				 ...
				 else
				 {
					 ...
					 $result2 = $this->bean->db->query($Q2);
						while ($row2 = $this->bean->db->fetchByAssoc($result2)) {
    ...
    function fetchallCheckedContacts($quoteID)
	{   
	    $query = "SELECT * FROM quotes_saved_contacts WHERE quotes_id='".$quoteID."'";
	...
	function fetchallCheckedUncheckedContacts($checkedContacts, $contactDetails)
	{  
        $totalContacts = count($contactDetails); 
		 for($i=0; $i<$totalContacts; $i++)
		 {
		     if (in_array($contactDetails[$i]['id'], $checkedContacts)) {
					 $contactDetails[$i]['checkedState'] = 1;
				}
		 }  
		return $contactDetails;
	}	
	function fetchQuotesOtherAccountsContactsByQuoteID($quoteID)
	{
	     $accountID       = $this->bean->billing_account_id;
		 $account         = $this->bean->billing_account;
	     
		 $contactIDs      = $this->fetchContactIdsByAccountIDNew($accountID);
		 $contactDetails  = $this->fetchContactDetailsByContactID($contactIDs);
		 $checkedContacts = $this->fetchallCheckedContacts($quoteID);
		 $finalcontacts   = $this->fetchallCheckedUncheckedContacts($checkedContacts, $contactDetails); 
		  return $finalcontacts;
	}
	
	function fetchQuotesSavedContactsByQuoteID($quoteID)
	{ 
        global $db;
		if ( ! isset($db) ) 
		 { 
        ...
        $sql = "SELECT qsc.contacts_id, ctc.first_name, ctc.last_name, ctc.salutation, ctc.id, ctc.phone_work, ctc.phone_mobile, ctcst.opportunity_role_c, ctcst.contacttype_c, ctcst.contact_type_primary_billing_c, ctcst.contact_type_other_billing_c, ctcst.contact_type_primary_tech_c, ctcst.contact_type_additional_tech_c, ctcst.contact_type_other_c FROM quotes_saved_contacts as qsc INNER JOIN contacts as ctc ON qsc.contacts_id = ctc.id INNER JOIN contacts_cstm as ctcst ON ctc.id = ctcst.id_c WHERE quotes_id = '".$quoteID."' and ctc.deleted = 0 ORDER BY ctc.first_name, ctc.last_name desc"; 
		$res = $this->bean->db->query($sql);
		while($row3 = $this->bean->db->fetchByAssoc($res)){  
		    $tmpArray['contacts_id']   = $row3['contacts_id'];  
			$tmpArray['id']   = $row3['id'];  
			$tmpArray['contact_name']  = $row3['first_name'].' '.$row3['last_name'];
			$tmpArray['phone_work']    = $row3['phone_work'];
			$tmpArray['phone_mobile']  = $row3['phone_mobile']; 
			 if($row3['contacttype_c'] == "PrimaryBilling")
			   $tmpArray['contacttype'] = "Primary Billing";
			 if($row3['contacttype_c'] == "OtherBilling")
			   $tmpArray['contacttype'] = "Other Billing";
			 if($row3['contacttype_c'] == "PrimaryTechnical")
			   $tmpArray['contacttype'] = "Primary Technical";
			 if($row3['contacttype_c'] == "AdditionalTechnical")
			   $tmpArray['contacttype'] = "Additional Technical";
			 if($row3['contacttype_c'] == "Others")
			   $tmpArray['contacttype'] = "Others";
			 
			 $tmpArray['role'] = "";
			 if($row3['contact_type_primary_billing_c'] == 1)
			 {
				  $tmpArray['role'] .= "Primary Billing, ";
			 }
			 else
			 {
				  $tmpArray['role'] .= "";
			 ...
			 {
				  $tmpArray['role'] .= "Additional Billing, ";
			 }
			 else
			 {
				  $tmpArray['role'] .= "";
			 ...
			 {
				  $tmpArray['role'] .= "Primary Technical, ";
			 }
			 else
			 {
				  $tmpArray['role'] .= "";
			 ...
			 {
				  $tmpArray['role'] .= "Additional Technical, ";
			 }
			 else
			 {
				  $tmpArray['role'] .= "";
			 ...
			 {
				  $tmpArray['role'] .= "Others, ";
			 }
			 else
			 {
				  $tmpArray['role'] .= "";
			 }
			$tmpArray['opportunity_role_c'] = $row3['opportunity_role_c'];
			...
			$tmpArray['email'] = $emailAddr;  
            $lenfind = substr($tmpArray['role'], 0, strlen($tmpArray['role'])-2);
			$tmpArray['role'] = $lenfind;  
		...
		if($i == 0)
		{
			...
			$sqs_objects = array('product_name['.$i.']' =>
			 	...
			 	'form' => 'EditView',
                    		...
                    		'populate_list' => array('product_description['.$i.']', 'product_name['.$i.']', 'product_id['.$i.']','product_cost_price['.$i.']','product_list_price['.$i.']','unit['.$i.']','product_code['.$i.']'),
 ...
 $html .= "<td class='dataField'><input class='sqsEnabled' autocomplete='off' tabindex='116' readonly='readonly' type='text' name='product_name[$i]' id='product_name$i' size='16' maxlength='50' value='".$row['name']."' title=''><input type='hidden' name='product_id[$i]' id='product_id$i' value='".$row['product_id']."'><input type='hidden' name='product_description[$i]' id='product_description$i' value='".$row['description']."'></td>";
...
$html .= "<td class='dataField'><input type='hidden' name='product_deleted[]' id='product_deleted$i' value='0'><input type='hidden' name='product_quote_id[]' value='".$row['id']."'><button type='button' class='button' value='".$mod_strings['LBL_REMOVE_PRODUCT_LINE']."' tabindex='116' onclick='markProductLineDeleted($i)'><input type='hidden' name='is_service[]' value='0' /><img src='themes/default/images/id-ff-clear.png' alt='".$mod_strings['LBL_REMOVE_PRODUCT_LINE']."'></button></td>";
		  	$html .= "</tr>";
			$product_idRow = $row['product_id'];
			
			$sql2 = "SELECT description FROM aos_products WHERE id = '".$product_idRow."' AND deleted = 0";
  		    $result2 = $this->bean->db->query($sql2);
			while ($row2 = $this->bean->db->fetchByAssoc($result2)) {
			      $product_descr = $row2['description'];
			}
			if($row['quotes_product_description_c'] != '')
			{
			    $quotes_product_description_c = $row['quotes_product_description_c'];
			}
			else
			{
			    $quotes_product_description_c = $product_descr;
			}
		
             //echo "<pre>"; print_r($row); echo "</pre>";           
           		$html .= "<tr class='oddListRowS1' height='20'  id='product_note_line$i'>";
			$html .= "<td class='dataField' align='right' style='padding:0px;margin:0px;vertical-align:top'><table border='0' cellspacing='3' cellpadding='0' ><tr><td class='dataField' align='right' >".$mod_strings['LBL_PRODUCT_QUOTE_DESC']." :&nbsp;</td></td></tr><tr><td class='dataField' align='right' >".$mod_strings['LBL_PRODUCT_QUOTE_DESCRIPTION']."&nbsp;</td></td></tr><tr style='border: none !important;'><td style='border: none !important;' class='' align='right' >&nbsp;</td></td></tr><tr style='border: none !important;'><td style='border: none !important;' class='' align='right' >&nbsp;</td></td></tr><tr><td class='dataField' align='right' >".$mod_strings['LBL_PRODUCT_QUOTE_NOTE'].":&nbsp</td></tr></table></td>";
			$html .= "<td class='dataField' colspan='6'><table border='0' cellspacing='0' cellpadding='0' ><tr><td class='dataField' align='left' > <input type='hidden' name='product_descriptionFetch[$i]' id='product_descriptionFetch$i' value='".$product_descr."'><input type='text'id='quotes_product_name$i' name='quotes_product_name[]' value='".$row['quotes_product_name']."' /></td></tr><tr><td class='dataField' align='left' ><textarea tabindex='116' name='quotes_product_description[]' id='quotes_product_description$i' rows='2' cols='53'> ".$quotes_product_description_c."</textarea></td></tr><tr><td class='dataField' align='left'><textarea tabindex='116' name='product_note[]' id='product_note$i' rows='2' cols='53'>".$row['description']."</textarea></td></tr></table></td>";
...
$dbconfigVars = $dbconfig['db_host_name'].";".$dbconfig['db_user_name'].";".$dbconfig['db_password'].";".$dbconfig['db_name'];		  
		$html .= "<br>
		            <input type='hidden' id='dbconfigVars' name='dbconfigVars' value='$dbconfigVars' />
					<input type='hidden' id='totalAccountContacts' name='totalAccountContacts' value='' />
					<input type='hidden' id='checkforContacts' name='checkforContacts' value='' />
					<input type='hidden' id='dataHTML' name='dataHTML' value='' />
					<input type='hidden' id='selectedAccountContacts' name='selectedAccountContacts' value='0' />
	<input type='hidden' id='totaldescCount' name='totaldescCount' value='".$totaldescCount."' />			
           <input type='hidden' id='totalcheckuncheck' name='totalcheckuncheck' value='' />	
           <input type='hidden' id='quotesID4Ajax' name='quotesID4Ajax' value='".$this->bean->id."' />		   
		         ";   
		$tempdivHTMLtoadd = '';
		
		
		if( isset($this->bean->id) and $this->bean->id != '' )
		{    // Edit Quotes  
			//$contacts = $this->fetchQuotesSavedContactsByQuoteID($this->bean->id);
            $contacts = $this->fetchQuotesOtherAccountsContactsByQuoteID($this->bean->id); 			
			$totalContacts = count($contacts); 
			  
			if($totalContacts > 0)
			{ 
			   $tempdivHTMLtoadd .= '<div id="addcontacts">';
			   
			   $tempdivHTMLtoadd .= '<table cellpadding="0" cellspacing="0" border="0" width="126%"><tr><td colspan="6" style="font-size: 12pt; font-weight: bold;" valign="top">Please select the Contact(s) to show on the SOF template.</td></tr> <tr> <td colspan="6" style="font-size: 12pt; font-weight: bold;" valign="top">&nbsp;</td></tr> <tr> <th style="text-align: left;">&nbsp;</th> <th style="text-align: left;">Role</th> <th style="text-align: left;">Name</th> <th style="text-align: left;">Work</th> <th style="text-align: left;">Mobile</th> <th style="text-align: left;">Email</th> </tr>';   
				 foreach($contacts as $key=>$val)
		           { 	
                       ...
                       $tempdivHTMLtoadd .= '<tr> <td colspan="6" style="font-size: 12pt; font-weight: bold;" valign="top">&nbsp;</td></tr><tr><td valign="top" style="width: 4%">'; 
                         if($val['checkedState'] == 1)
                         {
                             $tempdivHTMLtoadd .= '<input type="checkbox" checked="checked" onclick="checkfor(this);" name="contactsdisplay_'.$key.'" id="contactsdisplay_'.$key.'" value="'.$val['id'].'"> '; 
						 }
						 else
						 {
                            $tempdivHTMLtoadd .= '<input type="checkbox" onclick="checkfor(this);" name="contactsdisplay_'.$key.'" id="contactsdisplay_'.$key.'" value="'.$val['id'].'"> ';  
						 }	 
			           $tempdivHTMLtoadd .= '</td><td valign="top" style="width: 14%">'.chunk_split($opportunity_role_c, 17, "<br />").'</td><td valign="top" style="width: 14%"> <a href="http://sugarcrm-dev.nextlevelinternet.com/index.php?module=Contacts&return_module=Contacts&action=EditView&record='.$val['id'].'"> '.$val['name'].' </a></td>';
					   $tempdivHTMLtoadd .= '<td valign="top" style="width: 14%">'.$val['phone_work'].'</td>';
					   $tempdivHTMLtoadd .= '<td valign="top" style="width: 14%">'.$val['phone_mobile'].'</td>';
					   $tempdivHTMLtoadd .= '<td valign="top" style="width: 40%">'.$val['email'].'</td>';
					   $tempdivHTMLtoadd .= '</tr>';
                   }	 
			   $tempdivHTMLtoadd .= '<tr> <td colspan="6" style="font-size: 12pt; font-weight: bold;" valign="top"> <input type="button" name="getContacts" id="getContacts" value="Update Account Contact List" onClick="addContactsByAccount();" /> </td></tr>';	   
			   $tempdivHTMLtoadd .= '</table>';			
			   $tempdivHTMLtoadd .= '</div>'; 
			}
			else
			{
			   $tempdivHTMLtoadd .= '<div id="addcontacts"></div>'; 
			}
		}
		else
		{   // Add Quotes 
		  ...
		  ?>
		    <script language="Javascript">
			     var newdivtoadd = '<?php echo $tempdivHTMLtoadd; ?><br>';
			</script>
		<?php		
