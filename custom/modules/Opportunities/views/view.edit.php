...
EOQ;
	...
	function displayJS() {
	         ...
	         if( isset($_REQUEST['account_id']) and $_REQUEST['account_id'] != '' )
                {
				        $account_idChck = $_REQUEST['account_id'];
						$accountNameRec = $leadsDetails['account_name']; 
				}
		 	 else
                {
                        $account_idChck = "";
			...
			echo '<script type="text/javascript"> var filter=/^[0-9]+$/;   account_name
                        var lead_id     = "'.$lead_id.'";
						var account_idChck     = "'.$account_idChck.'"; 
						var account_id  = "'.$account_id.'";
						var accountName = "'.$accountName.'";
						
			            var today = "'.date("m/d/Y").'";
						var today2 = "'.date("m-d-Y").'";
			...
			echo '$(document).ready(function() {  
					  ...
					  { 
							...
							$("input[id=name]").val(accountName+" "+today2); 
							$("#account_id").val(account_id); 
					  } 
					  if(account_idChck != "")
					  {  
					        var accountNameRec     = $("input[id=account_name]").val(); 
							$("input[id=name]").val(accountNameRec+" "+today2);  
					  }
			         var mrc = $("#amount").val();
